...
model; 
	...
	(c - bee * c(-1) ) ^ (-eeis)	= bbeta * ( (1 + bee - ddelta + r(+1)) * ((c(+1) - bee * c ) ^ (-eeis)) - (bee * (1 - ddelta + r(+1)))*((c(+2) - bee * c(+1)) ^ (-eeis)) ) ;
	
	// 2. First order equation for labor-leisure
	w * ( (c - bee * c(-1) ) ^ (-eeis) - bee * bbeta * ((c(+1) - bee * c ) ^ (-eeis)) )		= cchi * (l ^ (1 / eeta));
...
steady_state_model;
	...
	z		= 0;
	w		= (1 - aalpha) * y / l;
	cchi	= w * (1 - (bee * bbeta) ) * ( ( (1 - bee) * c ) ^ (-eeis) ) * (l ^ (-1 / eeta));
